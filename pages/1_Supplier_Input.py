...
consent = st.checkbox(
    "I agree: my data will be used to calculate emissions and for anonymised academic research at IIT Roorkee."
)
if not consent:
...
with st.form("supplier_form"):
    ...
    for fid, fname, funit in fuel_list:
        ...
        with c1:
            uses = st.selectbox("Do you use this?", ["No", "Yes"], key=f"use_{fid}")
        with c2:
            qty = st.number_input(f"Quantity ({funit})", min_value=0.0, value=0.0, key=f"qty_{fid}")
        with c3:
            rel = st.selectbox("Data Reliability", ["Metered/Invoiced", "Estimate", "Best available"], key=f"rel_{fid}")
    ...
    for vtype, fuel_opts in veh_rows:
        ...
        with c1:
            n = st.number_input("Number of Vehicles", min_value=0, step=1, key=f"n_{vtype}")
        with c2:
            vfuel = st.selectbox("Fuel Type", fuel_opts, key=f"f_{vtype}")
        with c3:
            if vfuel == "ev":
                st.info("EV — no fuel. Add charging electricity to Section C1.")
                vqty = 0.0
            else:
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        has_hvac = st.selectbox("HVAC / AC units on site?", ["No", "Yes"], key="has_hvac")
    with c2:
        ref_type = st.selectbox("Refrigerant Type", ["R-22", "R-410A", "R-32", "R-134a", "R-407C"], key="ref_type")
    with c3:
        ref_kg = st.number_input("Refrigerant Topped-Up (kg) — enter 0 if No", min_value=0.0, value=0.0, key="ref_kg")

    if has_hvac == "No":
        ref_kg = 0.0
        st.caption("No HVAC — refrigerant emissions set to zero.")
    else:
        gwp_map = {"R-22": 1810, "R-410A": 2088, "R-32": 675, "R-134a": 1430, "R-407C": 1774}
        gwp = gwp_map.get(ref_type, 0)
        if ref_kg > 0:
            co2e_preview = round(ref_kg * gwp / 1000, 2)
            st.caption(f"{ref_type} GWP = {gwp}. Your fugitive CO2e = {ref_kg} kg x {gwp} / 1000 = {co2e_preview} tCO2e")

    st.markdown("---")

    st.markdown("**MIG/MAG Welding — Process CO2**")
    c4, c5 = st.columns(2)
    with c4:
        has_weld = st.selectbox("MIG/MAG Welding on site?", ["No", "Yes"], key="has_weld")
    with c5:
        weld_kg = st.number_input("Welding CO2 Gas Consumed (kg) — enter 0 if No", min_value=0.0, value=0.0, key="weld_kg")

    if has_weld == "No":
        weld_kg = 0.0
        st.caption("No welding — process CO2 set to zero.")
    else:
        if weld_kg > 0:
            weld_co2e = round(weld_kg / 1000, 4)
            st.caption(f"For C25 mix multiply total kg by 0.25. Your welding CO2e = {weld_kg} kg / 1000 = {weld_co2e} tCO2e")

    st.markdown("---")

    st.markdown("**Data Reliability — Refrigerant and Welding Gas**")
    b3_reliability = st.selectbox(
        "How reliable is your refrigerant and welding gas data?",
        ["Actuals from bills provided", "Approximate Estimate", "Best available estimate"],
        key="b3_reliability"
    )
    ...
    c1, c2 = st.columns(2)
    with c1:
        grid_kwh = st.number_input(
            "Grid Electricity Consumed (kWh/year) *",
            min_value=0.0
        )
    with c2:
        c2_source = st.selectbox(
            "Data Source for Electricity",
    ...
    with c3:
        has_rec = st.selectbox(
            "Do you hold RECs from IEX?",
            ["No", "Yes"],
            key="has_rec"
        )
    with c4:
        rec_vol = st.number_input(
            "REC Volume purchased (RECs) — enter 0 if No",
            min_value=0,
            step=1,
            value=0,
            key="rec_vol",
            help="1 REC = 1 MWh = 1,000 kWh of renewable electricity. "
                 "RECs are purchased from Indian Energy Exchange (IEX). "
                 "Current market price approximately INR 2,500 per REC. "
                 "Purchasing RECs allows you to claim market-based zero Scope 2 "
                 "for the equivalent electricity volume."
        )
    with c5:
        solar_kwh = st.number_input(
            "Captive / Rooftop Solar Generation (kWh/year)",
            min_value=0.0,
            help="Enter total units generated by your rooftop or captive solar plant last year. "
                 "Check your solar inverter meter or generation report. "
                 "This will be deducted from your grid consumption for Scope 2 calculation."
        )

    if has_rec == "No":
        rec_vol = 0
        st.caption("No RECs held — market-based Scope 2 will equal location-based.")
    else:
        rec_kwh_equivalent = rec_vol * 1000
        if rec_vol > 0:
            st.caption(
                f"Your {rec_vol} RECs cover {rec_kwh_equivalent:,.0f} kWh of renewable electricity. "
                f"Estimated annual REC cost = INR {rec_vol * 2500:,.0f} "
                f"(at approximately INR 2,500 per REC)."
            )

    if solar_kwh > 0:
        st.caption(
            f"Your captive solar generates {solar_kwh:,.0f} kWh/year. "
            f"This will be deducted from your grid consumption before calculating Scope 2."
        )

    st.markdown("---")

    st.markdown("**Diesel Generator (DG Set)**")
    dg_use = st.selectbox(
        "DG Set Usage",
        ["No DG Set", "Emergency Only (< 20 hrs/yr)", "Regular Backup / Peak Shaving"],
        help="DG set diesel is a Scope 1 emission. "
             "Include all DG diesel consumption in Section B1 Diesel above. "
             "This question helps assess your power reliability risk."
    )
    st.caption(
        "Diesel consumed by DG sets should be included in Section B1 Diesel quantity above. "
        "This question is for power reliability assessment only."
    )
    ...
    with c2:
        ...
        E3_value = st.number_input("If Yes — Shadow Price (INR / tCO2)", min_value=0) if E3 == "Yes" else 0
    ...
    # OPTIONAL WATER AND WASTE
    with st.expander("Optional — Water & Waste Module (for full ESG Score)"):
        st.caption("Completing this section enables a comprehensive ESG Sustainability Score.")
        w1, w2 = st.columns(2)
        with w1:
...
if submitted:
    ...
    responses = {
        ...
        'B1_rel': b1_rel, 'B3_rel': b3_reliability, 'C2': c2_source,
