...
with st.form("supplier_form"):
    ...
    for fid, fname, funit in fuel_list:
        ...
        with c1:
            uses = st.selectbox(f"{fname}", ["No", "Yes"], key=f"use_{fid}")
        with c2:
            qty = st.number_input(f"Quantity ({funit})", min_value=0.0, key=f"qty_{fid}") if uses == "Yes" else 0.0
        with c3:
            rel = st.selectbox("Data Reliability", ["Metered/Invoiced", "Estimate", "Best available"], key=f"rel_{fid}") if uses == "Yes" else "N/A"
